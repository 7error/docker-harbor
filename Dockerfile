...
COPY --from=goharbor/chartmuseum-photon:v2.0.3 /home/chart/chartm /usr/local/bin/chartm
COPY --from=goharbor/clair-adapter-photon:v2.0.3 /clair-adapter/clair-adapter /usr/local/bin/clair-adapter
COPY --from=goharbor/clair-photon:v2.0.3 /home/clair/clair /usr/local/bin/clair
COPY --from=goharbor/harbor-core:v2.0.3 /harbor/harbor_core /usr/local/bin/harbor_core
COPY --from=goharbor/harbor-core:v2.0.3 /harbor/migrations /harbor/migrations
COPY --from=goharbor/harbor-core:v2.0.3 /harbor/views /harbor/views
COPY --from=goharbor/harbor-jobservice:v2.0.3 /harbor/harbor_jobservice /usr/local/bin/harbor_jobservice
COPY --from=goharbor/notary-server-photon:v2.0.3 /bin/migrate-patch /usr/local/bin/migrate-patch
COPY --from=goharbor/notary-server-photon:v2.0.3 /bin/migrate /usr/local/bin/migrate
COPY --from=goharbor/notary-server-photon:v2.0.3 /bin/notary-server /usr/local/bin/notary-server
COPY --from=goharbor/notary-signer-photon:v2.0.3 /bin/notary-signer /usr/local/bin/notary-signer
COPY --from=goharbor/notary-server-photon:v2.0.3 /migrations /migrations

COPY --from=goharbor/harbor-portal:v2.0.3 /usr/share/nginx/html /portal
COPY --from=goharbor/registry-photon:v2.0.3 /usr/bin/registry_DO_NOT_USE_GC /usr/local/bin/registry
COPY --from=goharbor/harbor-registryctl:v2.0.3  /home/harbor/harbor_registryctl /usr/local/bin/harbor_registryctl

COPY --from=goharbor/trivy-adapter-photon:v2.0.3 /usr/local/bin/trivy /usr/local/bin/trivy
COPY --from=goharbor/trivy-adapter-photon:v2.0.3 /home/scanner/bin/scanner-trivy /usr/local/bin/scanner-trivy
